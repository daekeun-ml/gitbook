---
description: '대상: 분산 학습 경험은 있지만 Slurm에서 처음 돌려보는 분'
---
...
#### 1.1 그럼 AWS ParallelCluster랑은 뭐가 다른가?

HPC를 해봤다면 **AWS ParallelCluster(Pcluster)** 로 이미 Slurm 클러스터를 EC2에 띄워봤을 겁니다. "그거랑 HyperPod-Slurm이 뭐가 다르지?"가 자연스러운 질문이죠. 넷 다 **Slurm 인터페이스(sbatch/srun)는 거의 똑같습니다.** 진짜 차이는 **"AWS가 어디까지 관리하고, 내가 어디까지 운영하나"** 입니다.

**먼저 "관리형(managed)"부터 정확히** — "managed"는 한 덩어리가 아니라 **관리 범위**에 따라 4단계입니다. ParallelCluster를 그냥 "관리형"이라 부르면 틀립니다(실은 셀프 관리).

| 단계                                           | 뜻 (AWS가 관리하는 범위)                                                                       |
| -------------------------------------------- | -------------------------------------------------------------------------------------- |
| **셀프 관리** (self-managed)                     | AWS는 **도구·템플릿·AMI만** 제공. 프로비저닝·헤드노드·스케줄러·OS 패치·노드 복구 **전부 내 책임**. AWS가 운영하는 컨트롤 플레인 없음 |
| **관리형 스케줄러** (managed scheduler)             | AWS가 **스케줄러 컨트롤러**(예: `slurmctld`)를 **서비스 계정에서 대신 운영**. 컴퓨트 노드(EC2)는 내 계정              |
| **관리형 인프라·복원력** (managed infra & resiliency) | AWS가 인프라 프로비저닝 + **결함 노드 자동 감지·교체(복원력)**. 단 **스케줄러 운영은 내가**(노드에 로그인해 Slurm 운영)         |
| **완전 관리** (fully managed)                    | 나는 **작업만 제출**, AWS가 프로비저닝·실행·정리까지 전부. **노드 소유·SSH·스케줄러 운영 없음**                         |

이제 네 서비스를 이 스펙트럼에 놓으면:

|              | ParallelCluster        | AWS PCS (참고)    | HyperPod-Slurm                    | (참고) SageMaker Training Jobs |
| ------------ | ---------------------- | --------------- | --------------------------------- | ---------------------------- |
| 관리 단계        | **셀프 관리**              | **관리형 스케줄러**    | **관리형 인프라·복원력**                   | **완전 관리**                    |
| 스케줄러 컨트롤러 운영 | 내 헤드노드                 | **AWS**(서비스 계정) | 내 헤드노드                            | (없음/작업 제출)                   |
| 노드 소유·로그인    | 내 계정, SSH ✅            | 내 계정, SSH ✅     | 내 계정, SSH ✅                       | ❌ (노드 접근 없음)                 |
| GPU 결함 자동 복구 | ❌ (도구 데몬이 하되 내 헤드노드에서) | ✅ (노드 자동 교체)    | ✅ **HMA·deep health check·자동 교체** | (AWS가 대행)                    |
| 주 대상         | 전통 HPC (CFD·유전체)       | HPC + ML 배치     | **대규모 ML 학습**                     | 단발 ML 학습 잡                   |
| 스케줄러         | Slurm / AWS Batch      | Slurm           | Slurm / EKS                       | (해당 없음)                      |

> **쉽게 말하면**:
>
> * **ParallelCluster = "내가 직접 운영하는" Slurm 클러스터.** AWS가 관리하는 서비스가 아니라 **오픈소스 도구**입니다 — 세팅은 도구가 자동화해 주지만, 헤드노드·OS 패치·복구는 다 내 계정 안에서 내 책임. (자가 복구 데몬 `clustermgtd`조차 **내 헤드노드 위에서** 돕니다.)
> * **HyperPod-Slurm = "인프라·복원력은 AWS, 스케줄러 운영은 나".** GPU가 죽으면 AWS가 자동 교체해 주지만, Slurm 자체는 내 클러스터의 헤드노드에서 돌고 내가 SSH로 들어가 `sbatch`/`srun`으로 운영합니다.
>
> 즉 **"기존 HPC Slurm을 최소 변경으로 옮긴다" → ParallelCluster**, **"수천 GPU로 몇 주씩 학습하는데 GPU 장애 자동 복구가 필수" → HyperPod**. (AWS 내부 가이드도 "on-prem Slurm 최소 변경 → ParallelCluster / 대규모 ML + GPU 장애 자동복구 → HyperPod"로 안내.)

> ❓ **흔한 오해 ①: "ParallelCluster도 AWS 서비스니 관리형이겠지"? — 아닙니다, 셀프 관리입니다.** 공식 정의가 _"AWS supported, **open source** cluster management **tool**"_ 이고, 헤드/컴퓨트 노드는 **내 계정의 EC2**(_"The head and compute nodes are Amazon EC2 instances"_)이며 과금도 _"you only pay for the AWS resources that are created"_ — **도구 사용료도, AWS가 운영하는 컨트롤 플레인도 없습니다.** 클러스터를 소유·운영하는 건 나입니다.

> ❓ **흔한 오해 ②: "그럼 HyperPod는 완전 관리(fully managed)겠네"? — 그것도 아닙니다.** AWS도 HyperPod를 "fully managed"가 아니라 **"resilient"**(복원력) 클러스터로 표현합니다. AWS가 하는 건 **인프라 프로비저닝 + 결함 노드 자동 복구**까지고, **Slurm 운영·사용자 관리·체크포인트·`--auto-resume=1` 옵트인은 내 몫**입니다. "작업만 던지면 되는" 완전 관리는 **Training Jobs**(→ AWS 문구: _"a fully managed machine learning service"_)입니다.

> ⚠️ 정확성 노트: **PCS**는 AWS가 `slurmctld`를 서비스 계정에서 운영하는 건 확인되나(_"launches the Slurm controller in a service-owned account"_), 컨트롤러의 **HA/자동 장애조치(failover) 여부는 공식 확인 못 함** — "고가용성"으로 단정하지 마세요. 또 PCS는 노드 자동 교체도 하므로 "관리형 스케줄러 + 인프라 복원력"을 겸합니다. ParallelCluster의 **AWS Batch 모드**는 컴퓨트를 관리형 Batch에 위임하는 부분 예외이나, 도구 자체가 셀프 관리인 큰 결론은 동일합니다.

출처: [ParallelCluster란(open source tool)](https://docs.aws.amazon.com/parallelcluster/latest/ug/what-is-aws-parallelcluster.html) · [AWS PCS란](https://docs.aws.amazon.com/pcs/latest/userguide/what-is-pcs.html) · [HyperPod 복원력](https://docs.aws.amazon.com/sagemaker/latest/dg/sagemaker-hyperpod-resiliency-slurm.html) · [SageMaker Training Jobs](https://docs.aws.amazon.com/sagemaker/latest/dg/how-it-works-training.html)
