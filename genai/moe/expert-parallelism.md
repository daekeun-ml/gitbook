...
<figure><img src="../../.gitbook/assets/expert-parallelism-00.png" alt=""><figcaption><p>MoE 모델에서 주로 사용하는 전문가 병렬화 (EP) 도식</p></figcaption></figure>

* **Send Queue**: Router가 토큰을 라우팅해서 GPU별로 보낼 데이터를 큐에 쌓음
* **All-to-All (NCCL 통신)**: 각 GPU의 SendQ → 다른 GPU의 RecvQ로 데이터 전송
* **Recv Queue**: 해당 GPU의 Expert들이 처리할 토큰을 수신
* **Experts**: 라우팅된 토큰을 처리
* **Combine**: 처리된 결과를 다시 합쳐서 다음 Transformer Layer로 전달

{% code title="단일 디바이스의 전문가 병렬화(EP) forward pass 수행 예시" %}
```python
...
수학적으로 $$T$$개의 토큰이 $$N$$개의 디바이스에 분산된 토큰 배치를 예시로 들어 보겠습니다. (초기에는 디바이스당 $$T/N$$개의 토큰), $$E$$개의 전문가도 이 $$N$$개의 디바이스에 분산되어 있습니다(디바이스당 $$E/N$$개의 전문가).

디바이스 $$i$$에 있는 토큰 $$x$$가 게이팅 네트워크 $$g(x)$$를 통과할 때, 토큰은 물리적으로 디바이스 $$k$$에 위치한 전문가 $$E_j$$에 할당될 수 있습니다. (여기서 $$k$$는 $$i$$와 다를 수 있습니다.) 전문가 $$E_j$$가 계산을 수행하기 위해 $$x$$가 필요하므로, $$x$$는 디바이스 $$i$$에서 디바이스 $$k$$로 전송되어야 합니다.

이 과정은 모든 디바이스의 마이크로배치 내 모든 토큰에 대해 동시에 발생합니다. 각 디바이스 $$i$$는 자신의 토큰 중 서로 다른 부분 집합을 모든 다른 디바이스 $$k$$로 보낼 필요가 있으며(전문가가 로컬에 있을 경우 자신에게 보내는 경우도 포함), 대칭적으로 각 디바이스 $$k$$는 모든 다른 디바이스 $$i$$로부터 토큰을 받게 됩니다. $$S_{ik}$$를 현재 디바이스 $$i$$에 있는 토큰 중 디바이스 $$k$$에 있는 전문가에게 라우팅되어야 하는 토큰 집합이라고 할 때, All-to-All 통신은 모든 쌍 $$(i, k)$$에 대해 $$1 \le i, k \le N$$인 모든 $$S_{ik}$$ 집합의 전송을 수행합니다.

### 1.2. All-to-All의 난제

All-to-All 통신은 대역폭을 매우 많이 사용하기에 토큰당 연산량은 적은데 통신 빈도는 높은 MoE 모델의 분산 훈련에서 주요 병목이 됩니다. All-to-All은 일반적으로 로컬 배치 데이터의 상당 부분을 다른 노드로 전송하므로 총 통신량이 디바이스 수에 대해 거의 선형적으로 확장되기 때문입니다. (토큰 크기 × 배치 크기 × 시퀀스 길이 × 2\[dispatch + combine])실제 연구 결과에 따르면, 세 가지 주요 MoE 모델의 훈련 과정을 일반적으로 사용되는 GPU 클러스터에서 평가한 결과, All-to-All 통신 비율이 평균 45% 정도를 차지한다는 것이 발견되었습니다 \[1]. 이는 훈련 효율성에 상당한 영향을 미치는 수치입니다.
...
4. **확장성**: 모델 규모가 커질수록 All-to-All 통신의 효율성이 급격히 감소합니다. 수천 개의 GPU를 사용하는 대규모 시스템에서의 확장성 문제는 특히 심각합니다.
...
* 네트워크의 병렬 레이어나 분기<sup>branch</sup>에서의 계산
