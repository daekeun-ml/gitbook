...
* 통신 라이브러리: NCCL(+RDMA: 노드 간<sup>inter-node</sup>), EP all-to-all 최적화(계층/2D 또는 통신-연산 overlap)
* DeepEP 적용 시 NVSHMEM 설치 ([NVSHMEM Installation Guide](https://docs.nvidia.com/nvshmem/release-notes-install-guide/install-guide/abstract.html)) - NVSHMEM 경로 (`NVSHMEM_DIR`) 미지정 시 인터노드/저지연 기능이 비활성화됩니다.
{% endhint %}
