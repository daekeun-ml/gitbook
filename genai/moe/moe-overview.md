...
출력 $$G(x)$$는 dense $$N$$차원 벡터로, 각 요소 $$G(x)_i$$는 토큰을 전문가 $$i$$에 할당하는 라우터의 신뢰도를 나타냅니다. 소프트맥스 함수이기 때문에 $$G(x)$$의 모든 요소의 합은 1입니다.
...
#### Dense 모델과 Sparse MoE 모델의 계산 비용 (FLOPs) 비교

* **밀집 모델(Dense Models):** 토큰당 FLOPs는 밀집 계층 파라미터의 크기에 비례합니다. 계층 너비를 늘려 모델 용량을 확장하면 각 토큰에 대한 계산 비용이 직접 증가합니다. 입력/출력 차원이 $$d_{model}$$이고 중간 차원이 $$d_{ff}$$인 FFN의 경우, FLOPs는 대략 $$2 \times d_{model} \times d_{ff}$$입니다.
* **희소 MoE 모델(Sparse MoE Models):** 토큰당 FLOPs는 선택된 $$k$$개의 전문가 내 계산에 의해 지배됩니다. 각 전문가 $$E_i$$가 작은 밀집 FFN과 대략 동일한 계산 비용을 가진다면, 토큰당 총 계산 비용은 한 전문가 비용의 약 $$k$$배에 게이팅 네트워크 $$G(x)$$의 작은 오버헤드를 더한 값입니다. 중요한 점은 전문가 총 수 $$N$$를 (예: 16에서 64로) 늘려도 $$k$$가 일정하다면 토큰당 FLOPs는 증가하지 않는다는 것입니다. 이러한 분리는 MoE 모델이 동일한 토큰당 계산 예산 내에서 밀집 모델보다 훨씬 더 많은 파라미터 수와 표현 능력을 극적으로 확장할 수 있게 합니다.
...
MoE 모델 훈련 중에는 전문가 붕괴 현상이 자연스럽게 발생하는 경우가 많습니다:
...
1. **일반 범위**: 값은 보통 $$10^{-2}$$ 또는 $$10^{-3}$$처럼 작지만, 최적 값은 모델 아키텍처, 작업, 전문가 수, 선택한 보조 손실 공식에 크게 의존합니다.
2. **모니터링**: 훈련 중 전문가 활용 지표를 관찰합니다. 각 전문가가 훈련 단계나 에포크별로 처리하는 토큰 수를 추적합니다. Weights & Biases나 텐서보드 같은 툴킷이 매우 유용합니다. 또한 $$L_{aux}$$가 $$L_{task}$$에 비해 어느 정도인지도 모니터링합니다.
3. **반복 조정**: 작은 $$\alpha$$로 시작합니다. 만약 심각한 불균형(예: 일부 전문가는 평균보다 훨씬 적은 토큰을 계속 받는 경우)이 관찰되면 $$\alpha$$를 점진적으로 증가시킵니다. $$\alpha$$를 키울 때 주 작업 성능(검증 손실/정확도)이 크게 떨어지지 않도록 주의합니다.
